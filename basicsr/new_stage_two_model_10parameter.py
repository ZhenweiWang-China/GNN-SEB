...
class first_GNN(nn.Module):
    def __init__(self, height, width):
        ...
        self.downsample1 =PatchMerging(input_resolution=(height, width), dim=10, out_dim=20)#一般out_dim=6为dim的两倍
        self.downsample2 =PatchMerging(input_resolution=(height//2, width//2), dim=20, out_dim=40)#一般out_dim=6为dim的两倍
        ...
        self.upsample2 =UpsamplePixelShuffle(40)
        self.upsample1 = UpsamplePixelShuffle(20)

        self.con11 = Conv(10, 10)

        self.con1 = Conv(20, 20)
        #self.con2 = Conv(6, 6)
        self.con3 = Conv(10, 10)
        self.con4 = Conv(10, 10)
        self.con5 = nn.Conv2d(
                            in_channels=10,   # 输入通道数（对应输入的第二个维度 9）
        ...
        self.ASSA1 = BasicASTLayer(dim=10,
                            output_dim=10,
        ...
        self.ASSA2 = BasicASTLayer(dim=20,
                                   output_dim=20,
        ...
        self.ASSA3 = BasicASTLayer(dim=40,
                                   output_dim=40,
    ...
    def forward(self, x):
        ...
        y = x[:, 10, :, :] # 提取前12个波段 (shape: 9, H, W),提取数字-1个波段
        y = y.unsqueeze(1)
        #print(y.shape)
        #y = x[14, :, :]
        y=self.con6(y)
        ...
        x = x[:, :10, :, :]  # 提取前12个波段 (shape: 9, H, W),提取数字-1个波段
        #x = x[:14, :, :]
        x = self.con11(x)
        #print(x.shape)
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 11,32, 32)).cuda().to(torch.float32)
