...
class first_GNN(nn.Module):
    def __init__(self, height, width):
        ...
        self.downsample1 =PatchMerging(input_resolution=(height, width), dim=9, out_dim=18)#一般out_dim=6为dim的两倍
        self.downsample2 =PatchMerging(input_resolution=(height//2, width//2), dim=18, out_dim=36)#一般out_dim=6为dim的两倍
        self.downsample3 =PatchMerging(input_resolution=(height//4, width//4), dim=36, out_dim=72)#一般out_dim=6为dim的两倍

        self.upsample3 =UpsamplePixelShuffle(72)
        self.upsample2 =UpsamplePixelShuffle(36)
        self.upsample1 = UpsamplePixelShuffle(18)

        self.con11 = Conv(9, 9)

        self.con1 = Conv(18, 18)
        #self.con2 = Conv(6, 6)
        self.con3 = Conv(9, 9)
        self.con4 = Conv(9, 9)
        self.con5 = nn.Conv2d(
                            in_channels=9,   # 输入通道数（对应输入的第二个维度 9）
        ...
        self.ASSA1 = BasicASTLayer(dim=9,
                            output_dim=9,
        ...
        self.ASSA2 = BasicASTLayer(dim=18,
                                   output_dim=18,
    ...
    def forward(self, x):
        ...
        print(z.shape)
        fifth_result = self.fifth_GNN(con1) + z  # 输出为18个channel
        ...
        con4 = self.con3(sixth_result)+self.ASSA1(x)
        #print('con4', con4)
        con5 = self.con5(con4)
        #print('con5', con5)
        #on5 = con5 * 150 + 200
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 9, 16, 16)).cuda().to(torch.float64)
    #print('x', x)
    y = model(x)
