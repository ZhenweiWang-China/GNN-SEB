...
from base_models.swin_transformer import PatchEmbed
...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, num_feat, squeeze_factor=16):
        ...
        self.attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),#对输入信号，提供2维的自适应平均池化操作 对于任何输入大小的输入，可以将输出尺寸指定为H*W，但是输入和输出特征的数目不会变化。
...
class CAB(nn.Module):
    '''
    这个代码实现了一个 通道注意力模块（Channel Attention Block, CAB），
    其目的是通过卷积操作和通道注意力机制来增强特征表示，尤其是在处理图像特征时，
    能够更好地关注重要的通道（特征通道）。这个模块通常用于提高深度学习模型的表现，
    尤其是在图像重建、图像超分辨率或图像分类等任务中。
    '''

    def __init__(self, num_feat, compress_ratio=3, squeeze_factor=30, conv_type=''):
        super(CAB, self).__init__()
        '''
        num_feat：输入的特征维度，即输入的通道数。
        compress_ratio：压缩比，用于在卷积操作中减少中间层的通道数，帮助降低计算量并加强网络的表现。
        squeeze_factor：在通道注意力机制中使用的因子，控制特征压缩的程度，影响注意力的强度。
        conv_type：定义卷积类型，决定使用什么样的卷积层。如果 conv_type 为空，则使用默认的两层卷积
        加通道注意力。如果传入了 conv_type，则根据该类型选择不同的卷积块。
        '''
    ...
    def block_selection(self, conv_type:str):
        ...
        for name in conv_types:
            if name == 'ca':
                ...
                #dwconv：深度卷积（depthwise convolution）
...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    """
    这是在深度神经网络训练过程中用于正则化的一种方法，通常应用于卷积神经网络（CNN）或 Transformer 等深度模型中。
    DropPath 是随机丢弃整个路径或子网络（如某一层或者某一分支）。当某一层或某一分支被“丢弃”时，该层的输出将直接
    传递到下一层，跳过该层的计算。这种机制有助于减少模型的过拟合，促进模型的多样性和鲁棒性。
...
class Mlp(nn.Module):
    '''
    通常用于神经网络中的前馈层。它包含两个全连接层（Linear 层），一个激活函数（如 GELU 或 ReLU），以及 Dropout 正则化。
    '''
...
class dwconv(nn.Module):
    '''
    这个代码定义了一个 深度可分离卷积（Depthwise Convolution） 的模块，通常用于卷积神经网络中，
    以减少模型的计算复杂度和参数量。该模块通过使用一个深度可分离卷积层，结合激活函数（如 GELU）来进行特征提取。
    '''
...
class ConvFFN(nn.Module):
    ...
    def flops(self, n):
        '''
        flops 函数计算模型的 浮点运算量（FLOP），即模型在前向传播时所需的计算量。
        '''
...
class GAL(nn.Module):
    ...
    def forward(self, x, x_size, graph):
        ...
        # regroup
        '''
        对张量（tensor）的维度进行重新排列。具体来说，这段代码是用于将输入张量 x 重新排列成不同的形状，以便进行后续的操作，
        与reshape功能相似，rearrange可以自定义reshape的方向
        '''
...
class BasicLayer(nn.Module):
    '''
    这个类定义了一个神经网络中的基本层（或块），其功能和作用是在网络中构建具有多个子模块的层，可能包含自注意力、
    前馈网络等组成部分。这个类的设计与 Transformer 模型中的多层架构相似，可能是用于图像处理或其他类似领域的
    深度学习任务。
    '''
        ...
        for i in range(depth):
            if stages[i] == 'GN':
                block = GAL(
                    ...
                    sampling_method=0, # flag controlling local/global sampling只有这一行有差异
                    mlp_ratio=mlp_ratio,
                    bias=bias,
                    drop=drop,
    ...
    def forward(self, x, x_size, graph):
        for blk in self.blocks:
            if self.use_checkpoint:
                '''
                减少内存使用：通过丢弃中间的计算结果（梯度），在反向传播时重新计算这些中间结果，从而节省内存。
                提高训练效率：虽然可能会增加一些计算开销（因为需要重新计算中间结果），但是它能显著降低训练时所
                需的 GPU 内存，尤其是在处理非常大的模型时。
                '''
...
class PatchEmbed(nn.Module):
    r""" Image to Patch Embedding  图像到补丁嵌入

    作用:图像进行分割为patch，patch转tokens
    图像分块:
    将大图像分解成较小且均匀分布的小区域（patches）。这样做可以帮助模型更好地理解和学习图像的不同部分。
    特征提取:
    对每个 patch 应用卷积操作，从而提取出有意义的特征。这些特征随后会被转换为固定的低维向量表示。
    降维和标准化:
    通过对 patch 进行适当的变换，确保它们具有相似的尺度和特性，以便后续处理阶段更容易收敛和训练。
    适配 Transformer 架构:

    将二维图像数据转换为一维序列，使得传统基于自注意力机制的 Transformer 模型可以直接处理图像数据。
    Args:
...
class PatchUnEmbed(nn.Module):
    ...
    图像到补丁解除嵌入
    作用:反向重构图像:
    将一系列嵌入向量（tokens）重新排列成图像的 patch。
    合并 Patches:
    将各个 patch 合并成完整的图像，恢复其原有的空间结构。
    适应下游任务:
    在某些深度学习模型中，尤其是在图像重建任务中，最终可能需要将特征图转换回像素级别的图像表示。
    Args:
        img_size (int): Image size.  Default: 224.
        patch_size (int): Patch token size. Default: 4.
        in_chans (int): Number of input image channels. Default: 3.
...
class Upsample(nn.Sequential):
    """Upsample module. 输出是图像

    假设输入图像的尺寸为 (B, 64, H, W)（其中 B 为批大小），输出的尺寸会变成 (B, 64, 2H, 2W)，即输入图像的宽和高都被放大了 2 倍，且通道数保持不变。
    空间分辨率提高2倍
    Args:
...
class UpsampleOneStep(nn.Sequential):
    ...
    UpsampleOneStep模块（与Upsample的不同之处在于它总是只有1conv + 1pixelshuffle）
...
class IPG(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_layers = len(depths)#表示depths的长度，如果有6个元素（depths=[1, 1, 1, 1, 1, 1]），就等于6，这里表示MGB的数量
        self.embed_dim = embed_dim
        self.ape = ape
        ...
        # split image into non-overlapping patches  将图像分割成非重叠的补丁  patch to token
        self.patch_embed = PatchEmbed(
            img_size=img_size,
        ...
        # merge non-overlapping patches into image   合并不重叠的补丁到图像  token to patch
        self.patch_unembed = PatchUnEmbed(
            img_size=img_size,
            patch_size=patch_size,
            in_chans=embed_dim,
        ...
        # build the last conv layer in deep feature extraction  构建深度特征提取的最后一个转换层
        if resi_connection == '1conv':
    ...
    def forward(self, x):
        '''
        Set index & save input  设置索引和保存输入
        '''
        if (self.output_folder is not None):
            global list_to_save
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, out_dim, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = nn.Conv2d(4 * dim, out_dim, kernel_size=1, bias=False)  # Use Conv2d for image output
        #self.norm = norm_layer(4 * dim)
    ...
    def forward(self, x):
        ...
        # Split into 4 patches (B, H/2, W/2, C) for each patch
        x0 = x[:, 0::2, 0::2, :]  # B, H/2, W/2, C
        x1 = x[:, 1::2, 0::2, :]  # B, H/2, W/2, C
        x2 = x[:, 0::2, 1::2, :]  # B, H/2, W/2, C
        x3 = x[:, 1::2, 1::2, :]  # B, H/2, W/2, C

        # Concatenate the patches along the channel dimension (B, H/2, W/2, 4*C)
        x = torch.cat([x0, x1, x2, x3], -1)  # B, H/2, W/2, 4*C

        # Reshape back to (B, 4*C, H/2, W/2)
        x = x.permute(0, 3, 1, 2)  # B, 4*C, H/2, W/2

        # Apply normalization and reduction
        #x = self.norm(x)
        #x = self.norm(x.view(B, -1, H // 2, W // 2))  # Normalize over the channel dimension 修改的地方
        x = x.view(B, 2*C, H // 2, W // 2)
        print('x', x.shape)
        x = self.norm(x)  # 重新整理维度以适应归一化
