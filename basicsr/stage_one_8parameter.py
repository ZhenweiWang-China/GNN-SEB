...
class first_GNN(nn.Module):
    def __init__(self, height, width):
        ...
        self.downsample1 =PatchMerging(input_resolution=(height, width), dim=8, out_dim=16)#一般out_dim=6为dim的两倍
        self.downsample2 =PatchMerging(input_resolution=(height//2, width//2), dim=16, out_dim=32)#一般out_dim=6为dim的两倍
        self.downsample3 =PatchMerging(input_resolution=(height//4, width//4), dim=32, out_dim=64)#一般out_dim=6为dim的两倍

        self.upsample3 =UpsamplePixelShuffle(64)
        self.upsample2 =UpsamplePixelShuffle(32)
        self.upsample1 = UpsamplePixelShuffle(16)

        self.con11 = Conv(8, 8)

        self.con1 = Conv(16, 16)
        #self.con2 = Conv(6, 6)
        self.con3 = Conv(8, 8)
        self.con4 = Conv(8, 8)
        self.con5 = nn.Conv2d(
                            in_channels=8,   # 输入通道数（对应输入的第二个维度 9）
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 8, 32, 32)).cuda().to(torch.float32)
    #print('x', x)
    y = model(x)
