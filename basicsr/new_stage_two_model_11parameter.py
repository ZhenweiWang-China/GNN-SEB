...
class first_GNN(nn.Module):
    def __init__(self, height, width):
        ...
        self.downsample1 =PatchMerging(input_resolution=(height, width), dim=11, out_dim=22)#一般out_dim=6为dim的两倍
        self.downsample2 =PatchMerging(input_resolution=(height//2, width//2), dim=22, out_dim=44)#一般out_dim=6为dim的两倍
        ...
        self.upsample2 =UpsamplePixelShuffle(44)
        self.upsample1 = UpsamplePixelShuffle(22)

        self.con11 = Conv(11, 11)

        self.con1 = Conv(22, 22)
        #self.con2 = Conv(6, 6)
        self.con3 = Conv(11, 11)
        self.con4 = Conv(11, 11)
        self.con5 = nn.Conv2d(
                            in_channels=11,   # 输入通道数（对应输入的第二个维度 9）
        ...
        self.ASSA1 = BasicASTLayer(dim=11,
                            output_dim=11,
        ...
        self.ASSA2 = BasicASTLayer(dim=22,
                                   output_dim=22,
        ...
        self.ASSA3 = BasicASTLayer(dim=44,
                                   output_dim=44,
    ...
    def forward(self, x):
        ...
        y = x[:, 11, :, :] # 提取前12个波段 (shape: 9, H, W),提取数字-1个波段
        y = y.unsqueeze(1)
        #print(y.shape)
        #y = x[14, :, :]
        y=self.con6(y)
        ...
        x = x[:, :11, :, :]  # 提取前12个波段 (shape: 9, H, W),提取数字-1个波段
        #x = x[:14, :, :]
        x = self.con11(x)
        #print(x.shape)
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 12,32, 32)).cuda().to(torch.float32)
