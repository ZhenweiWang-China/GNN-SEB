...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        '''try:
            B, C, H, W = x.shape
            x = x.view(B, C, H * W)  # (1, 9, 256)

            # 2. 调整维度顺序：转换为 (B, L, C)
            x = x.permute(0, 2, 1)  # (1, 256, 9)
        except:
            pass'''
        # 根据张量维度决定处理逻辑
        if x.dim() == 4:
            # 处理 4D 输入 (B, C, H, W)
            B, C, H, W = x.shape
            x = x.view(B, C, H * W)  # (B, C, H*W) => 例如 (1, 9, 256)
            x = x.permute(0, 2, 1)  # 调整维度顺序 => (B, L, C) 例如 (1, 256, 9)
        elif x.dim() == 3:
            # 直接处理 3D 输入 (B, L, C) 或 (B, C, L)
            pass  # 无需额外操作
...
class BasicASTLayer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        #print(B, L, C)
        H = int(math.sqrt(L))
        # 1. 调整维度顺序：将 length=18 移到第二维度
        x = x.permute(0, 2, 1)  # 形状变为 [1, 18, 64]

        # 2. 分解 channels=64 为 8×8
        x = x.view(B, C, H, H)  # 最终形状 [1, 18, 8, 8]
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 18, 8, 8)).cuda().to(torch.float32)
    #print('x', x)
    y = model(x)
