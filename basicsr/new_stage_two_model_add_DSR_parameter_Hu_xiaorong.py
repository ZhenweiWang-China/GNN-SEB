...
class first_GNN(nn.Module):
    def __init__(self, height, width):
        ...
        self.first_GNN = IPG(
            ...
            depths=[4, 4, ],  # 里面的数（6）表示GAL的数量
            embed_dim=180,
            num_heads=[7, 7, ],#注意力机制的数量
            mlp_ratio=4,
        ...
        self.downsample1 =PatchMerging(input_resolution=(height, width), dim=14, out_dim=28)#一般out_dim=6为dim的两倍
        self.downsample2 =PatchMerging(input_resolution=(height//2, width//2), dim=28, out_dim=56)#一般out_dim=6为dim的两倍
        ...
        self.upsample2 =UpsamplePixelShuffle(56)
        self.upsample1 = UpsamplePixelShuffle(28)

        self.con11 = Conv(14, 14)

        self.con1 = Conv(28, 28)
        #self.con2 = Conv(6, 6)
        self.con3 = Conv(14, 14)
        self.con4 = Conv(14, 14)
        self.con5 = nn.Conv2d(
                            in_channels=14,   # 输入通道数（对应输入的第二个维度 9）
        ...
        self.ASSA1 = BasicASTLayer(dim=14,
                            output_dim=14,
        ...
        self.ASSA2 = BasicASTLayer(dim=28,
                                   output_dim=28,
        ...
        self.ASSA3 = BasicASTLayer(dim=56,
                                   output_dim=56,
        ...
        self.con6 = nn.Conv2d(
            in_channels=1,  # 输入通道数（对应输入的第二个维度 9）
            out_channels=3,  # 输出通道数（目标为 1）
            kernel_size=3,  # 卷积核尺寸（3x3）
            stride=1,  # 步长
            padding=1  # 填充，用于保持空间维度不变
        ).to(torch.float32)
        ...
        self.con7 = nn.Conv2d(
            in_channels=3,  # 输入通道数（对应输入的第二个维度 9）
            out_channels=1,  # 输出通道数（目标为 1）
            kernel_size=3,  # 卷积核尺寸（3x3）
            stride=1,  # 步长
            padding=1  # 填充，用于保持空间维度不变
    ...
    def forward(self, x):
        ...
        y = x[:, 14, :, :] # 提取前12个波段 (shape: 9, H, W),提取数字-1个波段
        y = y.unsqueeze(1)
        #print(y.shape)
        #y = x[14, :, :]
        y=self.con6(y)
        ...
        x = x[:, :14, :, :]  # 提取前12个波段 (shape: 9, H, W),提取数字-1个波段
        #x = x[:14, :, :]
        x = self.con11(x)
        #print(x.shape)
        ...
        third_res = self.third_GNN(downsample2)#输出为36个channel

        #third_result =self.ASSA3(third_res)
        #print('third_result', third_result)
        upsample2 = self.upsample2(third_res)#输出为18个channel
        ...
        #print(z.shape)
        fifth_res = self.fifth_GNN(con1) + second_result
        #fifth_result = self.ASSA2(fifth_res)  # 输出为18个channel
        #fifth_result = self.fifth_GNN(con1)+self.ASSA2(second_result)#输出为18个channel
        #print('fifth_result', fifth_result)
        upsample1 = self.upsample1(fifth_res)#输出为9个channel
        ...
        con44 = self.con4(sixth_result)+x
        #con4 =self.ASSA1(con44)
        #print('con4', con4)
        con5 = self.con5(con44)
        #print('con5', con5.shape)
        #on5 = con5 * 150 + 200
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 15,32, 32)).cuda().to(torch.float32)
    #x = torch.randn(( 15, 32, 32)).cuda().to(torch.float32)
    #print('x', x)
    y = model(x)
